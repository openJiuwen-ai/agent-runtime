...
def _load_message_payload(ns: argparse.Namespace) -> dict[str, Any]:
    if ns.message_file and ns.message_json:
        raise ValueError("请只指定其一：--message-file 或 --message-json")
    if ns.message_file:
    ...
    try:
        ...
        raise ValueError(f"消息体不是合法 JSON: {e}") from e
    if not isinstance(data, dict):
        raise ValueError("消息 JSON 必须是对象（顶层为 dict）")
