...
#   --message-template-json '{"chat_id":"chat_{sid}","bot_id":"bot_{sid}",
#   "user_id":"u_{sid}","session_id":"sess_{sid}","channel_id":"web",
#   "req_method":"chat.send","params":{"session_id":"sess_{sid}",
#   "content":"hello","mode":"agent","query":"1+1等于几"},
#   "is_stream":true,"metadata":{"query":{},"method":"chat.send"}}'
...
def _parse_session_spec(s: str) -> SessionSpec:
    ...
    for part in s.split(","):
        ...
        if not part:
            continue
        if "=" not in part:
            raise ValueError(f"无效 --session 片段（缺 '='）: {part!r}")
        k, _, v = part.partition("=")
        fields[k.strip()] = v.strip()
    miss = {"id", "ttl", "msgs", "conc"} - set(fields)
    if miss:
        raise ValueError(f"--session 缺字段 {sorted(miss)}: {s!r}")
    try:
        return SessionSpec(
            sid=fields["id"],
            ttl=int(fields["ttl"]),
        ...
        raise ValueError(f"--session 数值字段非法: {s!r} ({e})") from e
...
def _render_message(template: str, sid: str, seq: int, rid: str) -> dict[str, Any]:
    ...
    try:
        ...
        raise ValueError(f"模板渲染后非合法 JSON: {e}; 渲染结果: {rendered}") from e
    if not isinstance(data, dict):
        raise ValueError("模板渲染后顶层必须是 JSON 对象")
    data["request_id"] = rid
    return data
...
def _parse_env(pairs: List[str]) -> dict[str, str]:
    ...
    for p in pairs:
        if "=" not in p:
            raise ValueError(f"无效 --env 格式（需 KEY=val）: {p!r}")
        k, _, v = p.partition("=")
        out[k] = v
    return out
...
async def _amain() -> int:
    ...
    if not args.session or len(args.session) < 1:
        raise ValueError("至少需要一个 --session（推荐 3 个，覆盖不同 ttl）")
