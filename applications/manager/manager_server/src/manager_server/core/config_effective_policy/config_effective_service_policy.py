...
from manager_server.schemas.config_effective_policy_schemas import (
...
_SERVICE_POLICY_PATH = "/api/v1/config-effective/service-policies"
...
async def push_service_policies_sync_to_gateway(
    ...
    """Gateway 注册后：将 MDB 中该实例全部 Service 策略 bulk push 到 GDB。"""
    ...
    for idx, policy in enumerate(policies):
        ...
        last = await gateway_request(
            jid,
            "POST",
            f"{_SERVICE_POLICY_PATH}/",
            _clean_policy_for_create(policy),
            revision=f"sync-service-policy:{idx}",
        )
    return last or {
        "revision": "sync-service-policy",
        "success_flag": True,
        "result": {"synced": 0},
        "transport": "http",
    }
...
class ConfigEffectiveServicePolicyService:
    ...
    async def list_policies(
        self,
        jiuwenclaw_id: str,
        query: ConfigEffectiveServicePolicyListQuery,
    ) -> dict[str, Any]:
        normalized = await validate_jiuwenclaw_id(self._handler, jiuwenclaw_id)
    ...
    async def update(
        ...
        await gateway_request(
            normalized,
            "PATCH",
            f"{_SERVICE_POLICY_PATH}/{policy_id}",
            dict(updates),
        )
