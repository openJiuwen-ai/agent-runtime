...
_AGENT_POLICY_PATH = "/api/v1/config-effective/agent-policies"
...
async def push_agent_policies_sync_to_gateway(
    ...
    """Gateway 注册后：将 MDB 中该实例全部 Agent 策略 bulk push 到 GDB。"""
    ...
    for idx, policy in enumerate(policies):
        ...
        last = await gateway_request(
            jid,
            "POST",
            f"{_AGENT_POLICY_PATH}/",
            _clean_policy_for_create(policy),
            revision=f"sync-agent-policy:{idx}",
        )
    return last or {
        "revision": "sync-agent-policy",
        "success_flag": True,
        "result": {"synced": 0},
        "transport": "http",
    }
...
class ConfigEffectiveAgentPolicyService:
    ...
    async def update(
        ...
        await gateway_request(
            normalized,
            "PATCH",
            f"{_AGENT_POLICY_PATH}/{policy_id}",
            dict(updates),
        )
