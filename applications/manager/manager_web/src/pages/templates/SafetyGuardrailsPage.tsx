...
import type { PermissionsTemplate } from '../../types';
...
): { enabled: boolean; permissionMode: string } {
  const enabled = body?.enabled !== false;
  const modeRaw = body?.permission_mode === 'strict' ? 'strict' : 'normal';
  return { enabled, permissionMode: modeLabel(modeRaw) };
}
...
export function SafetyGuardrailsPage() {
  ...
  const [sortBy, setSortBy] = useState<PermissionsTemplateSortField | ''>('');
  ...
  const handleSortChange = (field: PermissionsTemplateSortField, value: ColumnSortValue) => {
    if (value === '') {
      setSortBy('');
      setSortOrder('asc');
    } else {
  ...
  const [items, setItems] = useState<PermissionsTemplate[]>([]);
  const [modalOpen, setModalOpen] = useState(false);
  const [editing, setEditing] = useState<PermissionsTemplate | null>(null);
  const [delTarget, setDelTarget] = useState<PermissionsTemplate | null>(null);
  ...
  const toggleEnabled = async (row: PermissionsTemplate, enabled: boolean) => {
    ...
    try {
      await PermissionsTemplateApi.update(row.template_id, { enabled });
  ...
  return (
    <>
      <div className="flex min-w-0 flex-col gap-4">
        <div className="page-header w-full min-w-0 flex-wrap items-start gap-y-3">
          <div className="min-w-[7.5rem] max-w-[20rem] shrink-0 sm:max-w-[32rem]">
            <div className="page-title truncate" title={t('safetyGuardrails.title')}>
              {t('safetyGuardrails.title')}
            </div>
            <div className="page-subtitle truncate" title={t('safetyGuardrails.subtitle')}>
              {t('safetyGuardrails.subtitle')}
            </div>
          </div>
          <div className="flex min-w-0 flex-1 flex-wrap items-center justify-end gap-2">
            <ListSearchInput
              ...
              placeholder={t('safetyGuardrails.searchPlaceholder')}
                ...
                <table className="table w-max min-w-full">
                  <thead>
                    <tr>
                      <th>
                        <TableColumnSort
                          label={t('safetyGuardrails.templateName')}
                      ...
                      <th>
                        <TableColumnSort
                          label={t('safetyGuardrails.templateDescription')}
                  ...
                  <tbody>
                    {items.length === 0 ? (
                      ...
                      items.map((row) => {
                        const summary = bodySummary(row.body, permissionModeLabel);
                        return (
                            ...
                            <td className="whitespace-nowrap">
                              <div className="flex flex-col gap-1.5">
                                ...
                                <div className="flex flex-wrap gap-1">
                                  <span className="tag">
                                    {summary.enabled
                                      ? t('safetyGuardrails.bodyEnabled')
                                      : t('safetyGuardrails.bodyDisabled')}
                                  </span>
                                  <span className="tag">
                                    {t('safetyGuardrails.permissionModeTag', { value: summary.permissionMode })}
                                  </span>
                                </div>
                              </div>
                            </td>
      ...
      <ConfirmDialog
        open={!!delTarget}
        message={t('safetyGuardrails.deleteConfirm')}
        danger
        onConfirm={async () => {
          if (!delTarget) return;
          try {
            await PermissionsTemplateApi.remove(delTarget.template_id);
