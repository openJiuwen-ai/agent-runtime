    ...
    >
      <div className="flex flex-col gap-4">
        ...
        <div className="flex items-start justify-between gap-4">
          <div><div className="font-medium">允许 HTTP 地址</div><div className="mt-1 text-xs text-muted">HTTP 总开关。使用回环、内网或公网 HTTP 地址时，还需开启对应的地址开关。</div></div>
          <Switch checked={allowHttp} disabled={loading} aria-label="允许 HTTP 地址" onChange={setAllowHttp} />
        </div>
        <div className="flex items-start justify-between gap-4">
          <div><div className="font-medium">允许本地回环地址</div><div className="mt-1 text-xs text-muted">允许访问请求发起方所在环境的 127.0.0.1、::1 和 localhost。发现由 Manager 发起，派发由 Gateway 发起。</div></div>
          <Switch checked={allowLoopback} disabled={loading} aria-label="允许本地回环地址" onChange={setAllowLoopback} />
        </div>
        <div className="flex items-start justify-between gap-4">
        ...
        <div className="rounded-md bg-warning/10 px-3 py-2 text-xs text-warning">仅建议用于可信联调网络。容器中的 127.0.0.1 指向容器自身，不是宿主机。</div>
      </div>
    </Modal>
  );
}
