...
export function AgentTemplatesPage() {
  ...
  const handleSortChange = (field: AgentTemplateSortField, value: ColumnSortValue) => {
    if (value === '') {
      setSortBy('');
      setSortOrder('asc');
    } else {
  ...
  const toggleEnabled = async (row: AgentTemplate, enabled: boolean) => {
    ...
    try {
      await AgentTemplateApi.update(row.template_id, { enabled });
  ...
  return (
    <>
      <div className="flex min-w-0 flex-col gap-4">
        <div className="page-header w-full min-w-0 flex-wrap items-start gap-y-3">
          <div className="min-w-[7.5rem] max-w-[12rem] shrink-0 sm:max-w-[16rem]">
            <div className="page-title truncate" title={t('agentTemplate.title')}>
              {t('agentTemplate.title')}
            </div>
            <div className="page-subtitle truncate" title={t('agentTemplate.subtitle')}>
              {t('agentTemplate.subtitle')}
            </div>
          </div>
          <div className="flex min-w-0 flex-1 flex-wrap items-center justify-end gap-2">
            <ListSearchInput
              ...
              placeholder={t('agentTemplate.searchPlaceholder')}
                ...
                <table className="table w-max min-w-full">
                  <thead>
                    <tr>
                      <th>
                        <TableColumnSort
                          label={t('agentTemplate.templateName')}
                      ...
                      <th>
                        <TableColumnSort
                          label={t('agentTemplate.templateDescription')}
                          value={sortBy === 'description' ? sortOrder : ''}
                          options={sortOptions}
                          onChange={(value) => handleSortChange('description', value)}
                        />
                      </th>
                      <th>{t('agentTemplate.agentTags')}</th>
                      <th>{t('agentTemplate.templateRef')}</th>
                      <th>
                        <TableColumnFilter
                      ...
                      <th>
                        <TableColumnSort
                          label={t('agentTemplate.updatedAt')}
                      ...
                      <tr key={row.template_id}>
                        ...
                        <td className="whitespace-nowrap">
                          <div className="flex items-center gap-1 flex-wrap">
                            {(row.agent_tags ?? []).map((tag) => (
                              <span key={tag} className="tag">{tag}</span>
                            ))}
                            {!row.agent_tags?.length && <span className="text-muted text-xs">—</span>}
                          </div>
                        </td>
      ...
      <ConfirmDialog
        open={!!delTarget}
        message={t('agentTemplate.deleteConfirm', { name: delTarget?.template_name ?? '' })}
        danger
        onConfirm={async () => {
          if (!delTarget) return;
          try {
