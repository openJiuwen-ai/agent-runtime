              ...
              {items.length === 0 ? (
                ...
                items.map((row) => {
                  ...
                  <tr key={row.id}>
                    ...
                    <td className="min-w-[10rem] max-w-[18rem] align-top">
                      <div className="text-text-strong font-medium truncate" title={linkedServicePolicy?.policy_name}>
                        {linkedServicePolicy?.policy_name ||
                          (row.service_policy_id
                            ? t('policies.agent.servicePolicyUnavailable')
                            : '—')}
                      </div>
                      <div className="text-[11px] text-muted mono break-all" title={row.service_policy_id}>
                        {row.service_policy_id}
                      </div>
                    </td>
