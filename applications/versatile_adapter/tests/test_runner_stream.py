...
@pytest.mark.asyncio
async def test_workflow_skip_with_default_json_dumps_format(write_yaml):
    """json.dumps 默认输出（冒号后有空格）的 finish/runCompleted 帧也被正确过滤。"""
    runner = VersatileAdapterRunner(config_path=write_yaml())
    sse_lines = [
        'data: {"type": "rawData", "data": {"content": "x"}}',
        'data: {"type": "finish", "data": {"content": ""}}',
        'data: {"type": "runCompleted", "data": {"content": ""}}',
    ]

    with _patch_httpx(sse_lines):
        events = []
        async for ev in runner.run_async(
            target={"intent": "knowledge_qa", "conversation_id": "c-space"},
            headers={},
            params={},
            body={"custom_data": {}},
        ):
