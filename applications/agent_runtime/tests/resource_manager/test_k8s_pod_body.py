...
class _TestPodClient:
    """Supply a recording SDK and use the public, cluster-free rendering API."""

    def __init__(self, **kwargs):
        self.renderer = RealK8sPodClient(**kwargs)

    def build(self, pod_id, spec):
        with patch.object(self.renderer, "_client", _fake_client_module()):
            return self.renderer.build_pod_body(pod_id, spec)


@pytest.fixture(name="client")
def pod_client() -> _TestPodClient:
    return _TestPodClient()
...
def test_build_pod_body_without_sidecars_unchanged(client):
    ...
    pod = client.build("pod-1", _base_spec())
...
def test_build_pod_body_renders_full_sidecar(client):
    ...
    assert sec["capabilities"].kwargs["add"] == [
        "NET_ADMIN",
        "SYS_ADMIN",
    ]  # canonical 排序
    ...
    hp = [v for v in pod_spec["volumes"] if v.kwargs["name"] == "hp-jiuwenbox-0-0"][0]
    assert hp.kwargs["host_path"].kwargs == {"path": "/sys/fs/cgroup", "type": None}
    assert box["volume_mounts"][0].kwargs == {
        "name": "hp-jiuwenbox-0-0",
...
def test_build_pod_body_rejects_port_conflict(client):
    ...
    spec = _base_spec(
        sidecars=[dict(JIUWENBOX, ports=[{"name": None, "container_port": 8086}])]
    )
    with pytest.raises(DeployFailed, match="containers invalid"):
        client.build("pod-1", spec)
...
def test_build_pod_body_renders_main_container_mounts(client):
    ...
    assert [e.kwargs for e in cm_vol["items"]] == [{"key": "k1", "path": "config.yaml"}]
    ...
    mounts = {m.kwargs["mount_path"]: m.kwargs for m in main_kwargs["volume_mounts"]}
...
def test_build_pod_body_main_security_context(client):
    ...
    main = client.build("pod-1", spec).kwargs["spec"].kwargs["containers"][0].kwargs
    effective = {
        k: v for k, v in main["security_context"].kwargs.items() if v is not None
    }
    ...
    main = (
        client.build(
            "pod-1",
            _base_spec(main_container=_main(security_context={"run_as_user": 1000})),
        )
        .kwargs["spec"]
        .kwargs["containers"][0]
        .kwargs
...
def test_build_pod_body_node_name(client):
    ...
    pod = client.build("pod-1", _base_spec(node_name="ecs-38b3-0001"))
    assert pod.kwargs["spec"].kwargs["node_name"] == "ecs-38b3-0001"
    pod = client.build("pod-1", _base_spec())
    assert pod.kwargs["spec"].kwargs["node_name"] is None
    pod = client.build("pod-1", _base_spec(node_name=""))
    assert pod.kwargs["spec"].kwargs["node_name"] is None
...
def test_build_pod_body_pvc_same_claim_shared_across_containers(client):
    ...
    ps = client.build("pod-1", spec).kwargs["spec"].kwargs
...
def test_build_pod_body_pvc_shared_claim_read_only_first_wins(client):
    ...
    ps = client.build("pod-1", spec).kwargs["spec"].kwargs
    pvc_vol = [v for v in ps["volumes"] if "persistent_volume_claim" in v.kwargs][0]
...
def test_build_pod_body_renders_command_args(client):
    ...
    spec = _base_spec(
        main_container=_main(command=["/bin/agent"], args=["--port", "8086"])
    )
    main = client.build("pod-1", spec).kwargs["spec"].kwargs["containers"][0].kwargs
    ...
    pod = client.build("pod-1", _base_spec(sidecars=[sc]))
    box = pod.kwargs["spec"].kwargs["containers"][1].kwargs
...
def test_build_pod_body_renders_pod_fs_group(client):
    ...
    pod = client.build("pod-1", spec)
    assert pod.kwargs["spec"].kwargs["security_context"].kwargs == {"fs_group": 2000}
    # 缺省:不设
    pod2 = client.build("pod-1", _base_spec())
...
def test_build_pod_body_dedupes_shared_hostpath_across_containers(client):
    ...
    shared_hp = [
        {
            "host_path": "/root/chenhui/jiuwenclaw",
            "mount_path": "/app/jiuwenswarm",
            "read_only": False,
            "host_path_type": "Directory",
        }
    ]
    sc = dict(JIUWENBOX, host_path_mounts=shared_hp)  # 与主容器同源
    spec = _base_spec(main_container=_main(host_path_mounts=shared_hp), sidecars=[sc])
    pod = client.build("pod-1", spec)
    pod_spec = pod.kwargs["spec"].kwargs
    ...
    assert hp_vols[0].kwargs["host_path"].kwargs == {
        "path": "/root/chenhui/jiuwenclaw",
        "type": "Directory",
    }
    mounts_by_container = {
        c.kwargs["name"]: [m.kwargs["name"] for m in c.kwargs["volume_mounts"]]
...
def test_build_pod_body_hostpath_differs_by_type_not_shared(client):
    ...
    spec = _base_spec(main_container=_main(host_path_mounts=hp_a), sidecars=[sc])
    pod = client.build("pod-1", spec)
...
def test_build_pod_body_dedupes_shared_configmap_across_containers(client):
    ...
    spec = _base_spec(main_container=_main(configmap_mounts=cm), sidecars=[sc])
    pod = client.build("pod-1", spec)
    ...
    spec2 = _base_spec(main_container=_main(configmap_mounts=cm), sidecars=[sc_diff])
    pod2 = client.build("pod-2", spec2)
...
def test_enforce_refuses_missing_agentserver_secret_before_pod_creation():
    link = LinkMTLSConfig(
        mode=LinkMTLSMode.ENFORCE,
        identity=MTLSDeploymentIdentity("deployment", "binding", 1),
    )
    client = _TestPodClient(link_mtls_config=link)
    with pytest.raises(DeployFailed, match="AgentServer certificate Secret"):
        client.build("pod-1", _base_spec())


def test_build_pod_body_enforce_mounts_secret_only_into_main_container():
    link = LinkMTLSConfig(
        mode=LinkMTLSMode.ENFORCE,
        identity=MTLSDeploymentIdentity("deployment-1", "binding-1", 2),
        agentserver_secret="agentserver-link-tls",
    )
    client = _TestPodClient(link_mtls_config=link)
    ...
    pod = client.build(
        "pod-1",
        _base_spec(
            main_container=_main(security_context={"run_as_user": 0}),
            sidecars=[JIUWENBOX],
        ),
    )
    pod_spec = pod.kwargs["spec"].kwargs
    ...
    assert pod_spec["subdomain"] == "jiuwenclaw-agentserver"
    assert pod_spec["hostname"] == "pod-1"
    assert main.kwargs["readiness_probe"].kwargs["tcp_socket"].kwargs == {"port": 8086}
    assert "http_get" not in main.kwargs["readiness_probe"].kwargs
    env = {item.kwargs["name"]: item.kwargs["value"] for item in main.kwargs["env"]}
    assert env["JIUWENSWARM_LINK_MTLS_MODE"] == "enforce"
    assert env["JIUWENSWARM_LINK_MTLS_PROFILE"].endswith("/profile.json")
    assert env["JIUWENSWARM_LINK_MTLS_CERT_FILE"].endswith("/tls.crt")
    assert any(
        volume.kwargs.get("secret")
        and volume.kwargs["secret"].kwargs["secret_name"] == "agentserver-link-tls"
        for volume in pod_spec["volumes"]
    )
    secret = next(
        v.kwargs["secret"] for v in pod_spec["volumes"] if v.kwargs.get("secret")
    )
    assert secret.kwargs["default_mode"] == 0o400
    assert any(
        mount.kwargs["name"] == "jiuwenswarm-link-mtls"
        for mount in main.kwargs["volume_mounts"]
    )
    assert all(
        mount.kwargs["name"] != "jiuwenswarm-link-mtls"
        for mount in sidecar.kwargs["volume_mounts"]
    )
    # Multiple AgentServers in this binding receive the SAME AgentServer role
    # material from Runtime, not Runtime's own private key or a per-Pod identity.
    second = client.build(
        "pod-2",
        _base_spec(
            main_container=_main(security_context={"run_as_user": 0}),
    ...
    second_spec = second.kwargs["spec"].kwargs
    second_main = second_spec["containers"][0].kwargs
    second_env = {
        item.kwargs["name"]: item.kwargs["value"] for item in second_main["env"]
    }
    assert (
        second_env["JIUWENSWARM_LINK_MTLS_PROFILE"]
        == env["JIUWENSWARM_LINK_MTLS_PROFILE"]
    )
    assert any(
        volume.kwargs.get("secret")
        and volume.kwargs["secret"].kwargs["secret_name"] == "agentserver-link-tls"
        for volume in second_spec["volumes"]
    )


@pytest.mark.parametrize("uid", [None, 1000])
def test_private_material_helpers_preserve_image_uid_and_unrelated_volumes(uid):
    link = LinkMTLSConfig(
        mode=LinkMTLSMode.ENFORCE,
        identity=MTLSDeploymentIdentity("deployment", "binding", 1),
        agentserver_secret="agentserver-link-tls",
    ...
    pod = client.build(
        "pod-1",
        _base_spec(
            main_container=_main(security_context={"run_as_user": uid}),
            sidecars=[JIUWENBOX],
        ),
    )
    spec = pod.kwargs["spec"].kwargs
    main, box, helper = [c.kwargs for c in spec["containers"]]
    init = spec["init_containers"][0].kwargs
    assert spec.get("security_context") is None  # do not invent Pod fsGroup/chown
    for trusted in (init, helper):
        assert trusted["image"] == main["image"]
        assert trusted["security_context"].kwargs.get("run_as_user") == uid
        assert trusted["security_context"].kwargs["allow_privilege_escalation"] is False
        assert {v.kwargs["mount_path"] for v in trusted["volume_mounts"]} == {
            "/run/link-source",
            "/run/link-owned",
        }
        assert "env" not in trusted  # never copy business credentials
    assert init["command"][-1] == "--once"
    assert all(
        "link-mtls-source" not in m.kwargs["name"] for m in main["volume_mounts"]
    )
    assert all("link-mtls" not in m.kwargs["name"] for m in box["volume_mounts"])
    env = {e.kwargs["name"]: e.kwargs["value"] for e in main["env"]}
    assert env["JIUWENSWARM_LINK_MTLS_PROFILE"].endswith(
        "/private/identity/profile.json"
    )
    assert env["JIUWENSWARM_LINK_MTLS_KEY_FILE"].endswith("/private/identity/tls.key")
    tls_mount = next(
        m.kwargs
        for m in main["volume_mounts"]
        if m.kwargs["name"] == "jiuwenswarm-link-mtls"
    )
    assert tls_mount["read_only"] is True


@pytest.mark.parametrize("name", ["link-material-sync", "link-material-init"])
def test_reject_material_helper_name_collision(name):
    link = LinkMTLSConfig(
        mode=LinkMTLSMode.ENFORCE,
        identity=MTLSDeploymentIdentity("deployment", "binding", 1),
        agentserver_secret="tls",
    )
    client = _TestPodClient(link_mtls_config=link)
    with pytest.raises(DeployFailed, match="reserved"):
        client.build("pod-1", _base_spec(main_container=_main(name=name)))
