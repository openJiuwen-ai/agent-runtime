...
诚实边界:两类 Pod 回收口径不同——**在役(候选集内)busy Pod 无回收路径**(空 Pod pass 只处理无会话者,reconcile 只处理非候选);**日落(软摘)Pod 会被 reconcile(场景 L)转 idle 记账(不理会话数),排空截止时即使仍带活跃会话也被硬切**——这就是"有界优雅"的界(窗口 = 日落 + session_ttl,与排空窗口文档"过窗仍有会话才硬切"一致)。
