...
- NFS(与 PVC 同构,卷源 server/path 随模板级 volumes):`{server(必,≤256), path?(≤256 或 null=导出根), mount_path(必,绝对), read_only=False}`。
同一容器内 `mount_path` 不得重复(四类挂载列表一起查)→ 400。指纹不变式同 sidecars(默认 None + 空归一 None + 排序)。
...
- `parse_container_spec(item, where, role)`:role = 引用位置(主容器/sidecar)。主容器:ports 必有 `name="sse"`(可另有一个 `http`;无名/他名 → 400)、探针恒 httpGet(`tcpSocket`/`timeoutSeconds` → 400)、缺省落定与 `Template` 默认逐项相等(period 5);sidecar:ports 至多 1 个无名端口、探针缺省 {None, /health, 5, **10**, 3}。securityContext 主/sidecar 同一白名单(决策 B):`runAsUser/runAsGroup/privileged/capabilities/seccompProfile/appArmorProfile`(type ∈ {Unconfined→true, RuntimeDefault→false},Localhost → 400)。**不可表示即拒绝**(command/args/protocol/envFrom 双 ref 等 → 400,绝不静默丢弃)。
...
锁内逐 DB 存活 scope(幻影 scope 归扩散③ drain;模板缺失的悬挂 scope 跳过+WARNING):
  ⓪ 前置日落中间态检查:rm_facade.sunset_pending_pods(sid)——scope 注册 Pod 中
     ...
     deploy_ver——refresh 不改配置值,版本判定对 refresh 日落失明),再过滤掉
     仍在 SM 候选集内的(2026-09-15:日落 ZREM 与并发 follower REGISTER 竞态
     重入集的老代 Pod 还在合法服务,等它=等会话生命周期;在集不拦与 sync 闸门
     跳过在集 Pod 一致,放行后的全量软摘除会摘它出集交 reconcile/reclaim 收敛)。
     非空 → 409 CONFIG_SYNC_BUSY,拒绝时 DB/Redis 零副作用(回收由 reclaim
