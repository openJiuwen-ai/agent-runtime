...
不做：本地审计文件、管道行、脱敏、SIEM、真 OTEL 联调（后续可补 `03_otel_emit.py`）。
...
examples/
├── README.md
├── 01_basic_memory.py         # 最小闭环
└── 02_context_and_config.py   # ContextVar / apply_config / audit_*
```

| 文件 | 做什么 |
| --- | --- |
| `01_basic_memory.py` | 注入 `MemoryEmitter`，`bind_audit_context` + `log_audit`，打印 attributes |
| `02_context_and_config.py` | `apply_config`、显式字段覆盖 ContextVar、`audit_info/warn/error`、`otel.enabled=false` 降级 Noop |
