...
"""最小闭环：注入 MemoryEmitter → 绑定上下文 → log_audit → 用 logging 打印写出记录。
...
def _log_record(index: int, record: dict) -> None:
    ...
    logger.info(
        "%s",
        json.dumps(record["attributes"], ensure_ascii=False, indent=2, sort_keys=True),
    )
...
def main() -> int:
    ...
    logger.info("emitted %s record(s)", len(mem.records))
